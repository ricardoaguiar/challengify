...
const Chrome = ({ title, links, actions, children }) => {
    ...
    <div className="view">
      <header className="viewHeader">
        {leftLinkProps?.to && leftLinkText && (
          <Link {...leftLinkProps}>{leftLinkText}</Link>
        )}
        <h1>{title}</h1>
        {rightLinkProps?.to && rightLinkText && (
          <Link {...rightLinkProps}>{rightLinkText}</Link>
        )}
      </header>
      <div className="viewContent">{children}</div>
      {actions && <footer className="viewActions">{actions}</footer>}
    </div>
  );
};
